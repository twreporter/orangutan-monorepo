...
/**
 * Get the distance from the element's left edge to the document's left edge
 *
 * @param {HTMLElement} element
 * @returns {number}
 */
export function getXRelativeToDocument(element) {
  if (typeof window !== 'undefined') {
    const scrollLeft =
      window.pageXOffset || document.documentElement.scrollLeft || 0
    const xRelativeToViewport = element
      ? element.getBoundingClientRect().left || 0
      : 0
    return xRelativeToViewport + scrollLeft
  }
  return 0
}
